...
def non_bottleneck_block_1d(x, filters=None, stride=1, dilation_rate=1, name="non-bottleneck-1d", config=config.Config()):
    ...
    for dim in range(dims):
        ...
        strides = [1] * dims
        strides[dim] = stride
        x = conv(x, filters, kernel_size=tuple(kernel_size), stride=tuple(strides), dilation_rate=1, use_bias=True, name=join(name, "1", f"dim{dim}", "conv"), config=config)
        if dim == dims - 1:
    ...
    for dim in range(dims):
        ...
        x = conv(x, filters, kernel_size=tuple(kernel_size), stride=1, dilation_rate=tuple(dilation_rates), use_bias=True, name=join(name, "2", f"dim{dim}", "conv"), config=config)
        if dim == dims - 1:
