...
def squeeze_excite_channel(x, reduction, filters=None, name="squeeze_excite_channel", config=config.Config()):
    ...
    x = conv(x, filters // reduction, kernel_size=1, use_bias=True, name=join(name, "conv1"), config=config)
    x = act(x, config=config)
    x = conv(x, filters, kernel_size=1, use_bias=True, name=join(name, "conv2"), config=config)
...
def squeeze_excite_spatial(x, name="squeeze_excite_spatial", config=config.Config()):
    ...
    x = conv(x, 1, kernel_size=1, use_bias=True, name=join(name, "conv"), config=config)
    x = tf.keras.layers.Activation("sigmoid")(x)
