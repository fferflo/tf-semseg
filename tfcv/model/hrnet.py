...
def downsample(x, n, filters, skip_last_act, name=None, config=config.Config()):
    for i in range(n):
        x = conv(x, filters=filters if i == n - 1 else x.shape[-1], kernel_size=3, stride=2, bias=False, name=join(name, str(i), "conv"), config=config)
        x = norm(x, name=join(name, str(i), "norm"), config=config)
        if i < n - 1 or not skip_last_act:
            x = act(x, config=config)
    return x

def upsample(x, n, filters, name=None, config=config.Config()):
    x = conv(x, filters=filters, kernel_size=1, stride=1, bias=False, name=join(name, "conv"), config=config)
    x = norm(x, name=join(name, "norm"), config=config)
    x = tfcv.model.upsample.resize(x, stride=(2 ** n), method="bilinear", config=config)
    return x
...
def module(xs, block, num_units, filters, name=None, config=config.Config()):
    ...
    xs = [act(x, config=config) for x in new_xs]
...
def hrnet(x, num_units, filters, blocks, num_modules, stem=True, name=None, config=config.Config()):
    ...
    for branch_index in range(1, len(xs)):
        x = xs[branch_index]
        x = tfcv.model.upsample.resize(x, stride=2 ** branch_index, method="bilinear", config=config)
        slice_end = tf.concat([dest_shape[:-1], [x.shape[-1]]], axis=0)
        x = tf.slice(x, slice_begin, slice_end)
        xs[branch_index] = x
...
def hrnet_v2_w48(x, name=None, stem=True, config=config.Config()):
    return hrnet(x,
        ...
        blocks=[resnet.bottleneck_block_v1] + 3 * [resnet.basic_block_v1],
        num_modules=[1, 1, 4, 3],
        stem=stem,
        name=name,
        config=config
