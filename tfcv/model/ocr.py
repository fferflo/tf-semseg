...
def object_attention(token_features, regions_features, filters_qkv, name=None, config=config.Config()):
    ...
    query = token_features # [batch, dims, features]
    query = conv_norm_act(query, filters=filters_qkv, kernel_size=1, stride=1, name=join(name, "query", "1"), config=config)
    query = conv_norm_act(query, filters=filters_qkv, kernel_size=1, stride=1, name=join(name, "query", "2"), config=config) # [batch, dims, filters]
    ...
    token_features = transformer.multihead_attention(query, key, value, heads=1, config=config) # [batch, dims, features]
...
def ocr(x, regions, filters=512, filters_qkv=256, fix_bias_before_norm=True, name="ocr", config=config.Config()):
    ...
    regions_probs = tf.nn.softmax(regions_probs, axis=1) # [batch, dims, regions]
    ...
    x_tokens = transformer.tokenize(x) # [batch, dims, features]

    # Gather
    regions_features = tf.matmul(regions_probs, x_tokens, transpose_a=True) # [batch, regions, features]

    # Distribute
    context_features = object_attention(x_tokens, regions_features, filters_qkv=filters_qkv, name=join(name, "distribute"), config=config) # [batch, dims, features]
